...
const toCommand = (state: JobState): AuthoredContainerCommand => {
  ...
  const argv =
    state.command._tag === "ShellCommand"
      ? (["sh", "-lc", state.command.command] as const)
...
export const isWorkflowAuthoring = (value: unknown): value is WorkflowAuthoring =>
  isWorkflowDsl(value) || isAuthoredWorkflow(value)
...
export const Job = {
  ...
  secret: (name: string, key = name): JobModifier =>
    (job) =>
      updateJob(job, (state) => ({
        ...state,
        env: mergeEnv(state.env, { [name]: Secret.ref(key) }),
      })),
...
export const Input = {
  ...
  fromWorkflow: (inputName: string, as = inputName, options: { readonly metadata?: AuthoredMetadata } = {}): AuthoredUnitInputDeclaration =>
    publicInput(as, sourceFromWorkflowInput(inputName), options.metadata),
  fromJob: (jobId: string, outputName: string, as = outputName, options: { readonly metadata?: AuthoredMetadata } = {}): AuthoredUnitInputDeclaration =>
    publicInput(as, sourceFromJobOutput(jobId, outputName), options.metadata),
}
...
export const Output = {
  ...
  fromJob: (jobId: string, outputName: string, as = outputName, options: { readonly metadata?: AuthoredMetadata } = {}): AuthoredWorkflowOutputDeclaration =>
    publicOutput(as, sourceFromJobOutput(jobId, outputName), options.metadata),
}
...
export const Report = {
  file: (name: string, path: string, options: { readonly format?: AuthoredDataValueFormat; readonly contentType?: string; readonly metadata?: AuthoredMetadata } = {}): AuthoredReportDeclaration => {
...
export const Retry = {
  times: (
    ...
    const policy: AuthoredRetryPolicy = {
      _tag: "RetryPolicy",
      maxAttempts,
    }
    addOptional(policy, "exponent", options.exponent)
    addOptional(policy, "baseDelayMillis", options.baseDelayMillis)
    addOptional(policy, "maxDelayMillis", options.maxDelayMillis)
    addOptional(policy, "jitter", options.jitter)
    return policy
  },
}

export const Timeout = {
  seconds: (seconds: number) => ({ _tag: "TimeoutPolicy", seconds } as const),
  minutes: (minutes: number) => ({ _tag: "TimeoutPolicy", seconds: minutes * 60 } as const),
}

export const Cancellation = {
  bestEffort: (): AuthoredCancellationPolicy => ({ _tag: "CancellationPolicy", mode: "best-effort" }),
  failFast: (): AuthoredCancellationPolicy => ({ _tag: "CancellationPolicy", mode: "fail-fast" }),
}

export const Secret = {
  ref: (key: string) => new SecretRef({ key }),
}
