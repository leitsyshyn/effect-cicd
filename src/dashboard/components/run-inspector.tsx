...
export function RunInspector(props: {
  ...
  return (
    <section className="dashboard-section min-h-0 overflow-hidden">
      <header className="border-b border-border px-4 py-3 sm:px-5">
        <div className="flex items-start justify-between gap-4">
          <div className="grid gap-1">
            <div className="text-[17px] font-semibold text-foreground">{props.unit?.name ?? "Unit inspector"}</div>
            <div className="text-sm text-muted-foreground">{props.unit?.unitId ?? "Select a job from the pipeline or jobs view."}</div>
          </div>
      ...
      <div className="grid gap-4 px-4 py-4 sm:px-5">
        {props.unit === undefined ? (
          <EmptyState title="No unit selected" description="Choose a job to inspect logs, artifacts, attempts, and resolved values." compact />
        ) : (
          <>
            <div className="dashboard-subsection px-4 py-4">
              <dl className="dashboard-key-value grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <div className="mt-4 border-t border-border pt-4">
                ...
                <div className="mt-2 overflow-x-auto font-mono text-[12px] text-zinc-200">{props.unit.command ?? "Not retained"}</div>
              </div>
            </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="min-h-0">
          <TabsList className="grid w-full grid-cols-4 rounded-none border-0 bg-transparent p-0 dashboard-nav">
          ...
          <div className="h-[72vh] min-h-[560px] pt-2">
                ...
                <ScrollArea className="h-full pr-2">
                  <div className="grid gap-3">
                    ...
                    <InspectorBlock title="Reports" icon={<FileCode2 className="size-4 text-[var(--dashboard-highlight)]" />}>
                      {(props.unit.reports ?? []).length === 0 ? <MutedLine>No reports recorded.</MutedLine> : (props.unit.reports ?? []).map((report) => <ValueRow key={report.artifactRef} label={report.name} value={report.format} detail={truncateMiddle(report.artifactRef, 44)} />)}
                    </InspectorBlock>
                    ...
                    <InspectorBlock title="Unit timeline" icon={<Clock3 className="size-4 text-[var(--dashboard-highlight)]" />}>
                      {props.events.length === 0 ? <MutedLine>No unit-scoped events.</MutedLine> : props.events.map((event) => <ValueRow key={event.eventId} label={event.type} value={event.message} detail={formatDateTime(event.occurredAt)} />)}
                    </InspectorBlock>
                  </div>
                </ScrollArea>
              )}
                  ...
                  <div className="grid gap-3">
                    {props.unit.attempts.length === 0 ? (
                      ...
                      props.unit.attempts.map((attempt) => (
                        <div key={attempt.attemptId} className="dashboard-subsection px-4 py-3">
