...
export function RunHeader(props: {
  ...
  const workflowLabel =
    props.detail.run.workflowName ?? props.detail.run.workflowId;
    ...
    <div className="grid gap-4">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link to={hrefForProject(props.detail.run.projectId)}>
      ...
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-4">
        <div className="grid min-w-0 gap-3">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
            ...
            <span className="max-w-full break-all font-mono text-[12px]">
              {truncateMiddle(props.detail.run.runId, 72)}
            </span>
            {props.detail.source.retriedFromRunId === undefined ? null : (
              <Link
                to={hrefForRun(props.detail.source.retriedFromRunId)}
                className="hover:text-foreground"
              >
                retried from{" "}
                {truncateMiddle(props.detail.source.retriedFromRunId, 36)}
              </Link>
            )}
          </div>
          {props.detail.run.failureMessage === undefined ? null : (
            <p className="text-sm text-destructive">
              {props.detail.run.failureMessage}
            </p>
          )}
          {props.detail.run.cancellationReason === undefined ? null : (
            <p className="text-sm text-destructive">
              {props.detail.run.cancellationReason}
            </p>
          )}
          {props.actionError === undefined ? null : (
            <p className="text-sm text-destructive">{props.actionError}</p>
          )}
          {props.actionNotice === undefined ? null : (
        ...
        <div className="flex flex-wrap items-center gap-2">
          {props.detail.run.controls.canRetry ? (
            <Button
              ...
              disabled={props.actionPending !== undefined}
            >
          ...
          {props.detail.run.controls.canGc ? (
            ...
            >
              ...
              {props.actionPending === "gc" ? "Clearing..." : "Clear"}
            </Button>
          ) : null}
        </div>
      </div>
